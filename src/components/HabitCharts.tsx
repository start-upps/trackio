...
function prepareWeeklyData(habit: Habit) {
  ...
  return days.map(day => {
    ...
    const completed = habit.entries.some(
      entry => format(new Date(entry.date), 'yyyy-MM-dd') === dayStr && entry.completed
    )
...
function prepareCompletionRateData(habit: Habit) {
  ...
  while (currentDate <= endDate) {
    ...
    const weekEntries = habit.entries.filter(entry => {
      ...
      return entryDate >= weekStart && entryDate < startOfWeek(currentDate, { weekStartsOn: 1 })
    })
    
    const completionRate = weekEntries.length > 0
      ? (weekEntries.filter(e => e.completed).length / weekEntries.length) * 100
      : 0
...
export function HabitCharts({ habit }: HabitChartsProps) {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-gray-400">Weekly Completion Rate</h4>
        <div className="h-[200px] w-full">
          <ResponsiveContainer width="100%" height="100%">
